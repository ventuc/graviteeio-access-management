...
public class PermissionsFilter implements ContainerRequestFilter {
    ...
    private void filterResource(String referenceId, ReferenceType referenceType, Permission permission) {
        ...
        List<Membership> memberships = membershipService.findByReference(referenceId, referenceType).blockingGet();
        if (memberships ==  null || memberships.isEmpty()) {
            throw new ForbiddenException();
        }

        // get user groups
        List<Group> groups = groupService.findByMember(getAuthenticatedUser().getId()).blockingGet();
        ...
        if (!hasPermission(resourceMemberships, permission)) {
            throw new ForbiddenException();
        }
    }

    private boolean hasPermission(List<Membership> resourceMemberships, Permission permission) {
        List<String> roleIds = resourceMemberships.stream().map(Membership::getRole).collect(Collectors.toList());
        Set<Role> roles = roleService.findByIdIn(roleIds).blockingGet();
    ...
    private boolean isAdmin() {
        ...
        List<String> roles = (List<String>) user.getAdditionalInformation().get(CustomClaims.ROLES);
        return roles != null && roles.contains(AbstractResource.MANAGEMENT_ADMIN);
    }
